...
def render_sqx_us100_reversion_h1_ea(strategy_id: str, payload: dict[str, Any]) -> str:
    params = dict(payload.get("params", {}))
    symbol = payload.get("symbol", "US100.cash")
...
input int BBPeriod = {parse_int(params.get("BBPeriod1", 34), 34)};
input double BBDev = {parse_float(params.get("BBDev1", 2.2), 2.2)};
input int WPRPeriod = {parse_int(params.get("WPRPeriod1", 21), 21)};
input double WPRUpper = {parse_float(params.get("WPRUpper", -15.0), -15.0)};
input double WPRLower = {parse_float(params.get("WPRLower", -85.0), -85.0)};
input int ATRPeriod = {parse_int(params.get("ATRPeriod1", 14), 14)};
input double StopLossCoef = {parse_float(params.get("StopLossCoef1", 2.5), 2.5)};
input double ProfitTargetCoef = {parse_float(params.get("ProfitTargetCoef1", 1.8), 1.8)};

input group "Session"
input int SignalFromHour = {parse_int(str(params.get("SignalTimeRangeFrom", "01:00"))[:2], 1)};
input int SignalFromMinute = {parse_int(str(params.get("SignalTimeRangeFrom", "01:00"))[3:], 0)};
input int SignalToHour = {parse_int(str(params.get("SignalTimeRangeTo", "22:00"))[:2], 22)};
input int SignalToMinute = {parse_int(str(params.get("SignalTimeRangeTo", "22:00"))[3:], 0)};
input int FridayExitHour = {parse_int(str(params.get("FridayExitTime", "22:30"))[:2], 22)};
input int FridayExitMinute = {parse_int(str(params.get("FridayExitTime", "22:30"))[3:], 30)};
